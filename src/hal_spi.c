#include "esp_log.h"
#include "hal_esp32_defs.h"

#include "hal_common.h"
#include "hal_spi_types.h"
#include "hal_spi_master.h"

#include "esp_err.h"
#include "driver/spi_master.h"

static void hal_config_to_esp_config(struct hal_spi_config *config, spi_device_interface_config_t *esp_config) {
    esp_config->command_bits = 0;
    esp_config->address_bits = 0;
    esp_config->dummy_bits = 0;
    esp_config->clock_speed_hz = config->frequency_hz;
    esp_config->duty_cycle_pos = 128; // 50% duty cycle
    esp_config->cs_ena_pretrans = 0;
    esp_config->cs_ena_posttrans = 0;
    esp_config->flags = 0;
    esp_config->queue_size = 1; // Basic mode uses minimal queue
    esp_config->pre_cb = NULL;
    esp_config->post_cb = NULL;
    
    // Set SPI mode (CPOL and CPHA)
    switch (config->mode) {
        case HAL_SPI_MODE_0:
            esp_config->mode = 0;
            break;
        case HAL_SPI_MODE_1:
            esp_config->mode = 1;
            break;
        case HAL_SPI_MODE_2:
            esp_config->mode = 2;
            break;
        case HAL_SPI_MODE_3:
            esp_config->mode = 3;
            break;
        default:
            esp_config->mode = 0;
            break;
    }
    
    // Set bit order
    if (config->bit_order == HAL_SPI_BIT_ORDER_LSB_FIRST) {
        esp_config->flags |= SPI_DEVICE_BIT_LSBFIRST;
    }
    
    // Set CS pin
    esp_config->spics_io_num = config->impl_config->cs_pin;
}

static void hal_bus_config_to_esp_config(struct hal_spi_config *config, spi_bus_config_t *esp_bus_config) {
    esp_bus_config->mosi_io_num = config->impl_config->mosi_pin;
    esp_bus_config->miso_io_num = config->impl_config->miso_pin;
    esp_bus_config->sclk_io_num = config->impl_config->sclk_pin;
    esp_bus_config->quadwp_io_num = -1;
    esp_bus_config->quadhd_io_num = -1;
    esp_bus_config->max_transfer_sz = 0; // Use default for basic mode (no DMA)
    esp_bus_config->flags = SPICOMMON_BUSFLAG_MASTER;
}

hal_spi_result_t hal_spi_master_init(struct hal_spi_context *spi_ctx) {
    if (spi_ctx == NULL || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    if (spi_ctx->impl_ctx->is_initialized) {
        return HAL_SPI_OK;
    }
    
    // Initialize context to sync-only mode
    spi_ctx->current_mode = HAL_SPI_OP_MODE_SYNC_ONLY;
    
    // Create mutex for thread safety
    spi_ctx->impl_ctx->mutex = xSemaphoreCreateMutex();
    if (spi_ctx->impl_ctx->mutex == NULL) {
        return HAL_SPI_ERR_OTHER;
    }
    
    spi_ctx->impl_ctx->is_initialized = true;
    spi_ctx->impl_ctx->is_driver_installed = false;
    spi_ctx->impl_ctx->device_handle = NULL;
    
#if defined(HAL_SPI_ASYNC_DMA_SUPPORT)
    spi_ctx->impl_ctx->async_device_handle = NULL;
#endif
    
    return HAL_SPI_OK;
}

hal_spi_result_t hal_spi_master_deinit(struct hal_spi_context *spi_ctx) {
    if (spi_ctx == NULL || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    if (!spi_ctx->impl_ctx->is_initialized) {
        return HAL_SPI_OK;
    }
    
    BaseType_t mutex_ret_err = xSemaphoreTake(spi_ctx->impl_ctx->mutex, pdMS_TO_TICKS(1000));
    if (mutex_ret_err == pdTRUE) {
        // Remove device if attached
        if (spi_ctx->impl_ctx->device_handle != NULL) {
            esp_err_t ret_err = spi_bus_remove_device(spi_ctx->impl_ctx->device_handle);
            if (ret_err != ESP_OK) {
                xSemaphoreGive(spi_ctx->impl_ctx->mutex);
                return esp_err_to_spi_hal_err(ret_err);
            }
            spi_ctx->impl_ctx->device_handle = NULL;
        }
        
        // Free SPI bus if installed
        if (spi_ctx->impl_ctx->is_driver_installed) {
            esp_err_t ret_err = spi_bus_free(spi_ctx->spi_bus_id);
            if (ret_err != ESP_OK) {
                xSemaphoreGive(spi_ctx->impl_ctx->mutex);
                return esp_err_to_spi_hal_err(ret_err);
            }
            spi_ctx->impl_ctx->is_driver_installed = false;
        }
        
        // Clean up mutex and reset state
        SemaphoreHandle_t mutex_to_delete = spi_ctx->impl_ctx->mutex;
        spi_ctx->impl_ctx->is_initialized = false;
        spi_ctx->impl_ctx->mutex = NULL;
        
        xSemaphoreGive(mutex_to_delete);
        vSemaphoreDelete(mutex_to_delete);
        
        return HAL_SPI_OK;
    } else {
        return HAL_SPI_ERR_BUS_BUSY;
    }
}

hal_spi_result_t hal_spi_master_set_config(struct hal_spi_context *spi_ctx, struct hal_spi_config *config) {
    if (spi_ctx == NULL || config == NULL || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    esp_err_t ret_err;
    hal_spi_result_t spi_result = HAL_SPI_OK;
    spi_bus_config_t esp_bus_config = {0};
    spi_device_interface_config_t esp_device_config = {0};
    
    hal_bus_config_to_esp_config(config, &esp_bus_config);
    hal_config_to_esp_config(config, &esp_device_config);
    
    BaseType_t mutex_ret_err = xSemaphoreTake(spi_ctx->impl_ctx->mutex, pdMS_TO_TICKS(1000));
    if (mutex_ret_err == pdTRUE) {
        // Initialize SPI bus if not already done
        if (!spi_ctx->impl_ctx->is_driver_installed) {
            ret_err = spi_bus_initialize(config->spi_bus_id, &esp_bus_config, SPI_DMA_DISABLED);
            if (ret_err != ESP_OK) {
                spi_result = esp_err_to_spi_hal_err(ret_err);
                goto free_mutex_and_ret;
            }
            spi_ctx->impl_ctx->is_driver_installed = true;
        }
        
        // Add device to SPI bus
        ret_err = spi_bus_add_device(config->spi_bus_id, &esp_device_config, &spi_ctx->impl_ctx->device_handle);
        if (ret_err != ESP_OK) {
            spi_result = esp_err_to_spi_hal_err(ret_err);
            goto free_mutex_and_ret;
        }
        
        // Store actual frequency (ESP-IDF may adjust it)
        spi_ctx->actual_frequency_hz = config->frequency_hz; // ESP-IDF will adjust to closest supported
        
        free_mutex_and_ret:
            xSemaphoreGive(spi_ctx->impl_ctx->mutex);
            return spi_result;
    } else {
        return HAL_SPI_ERR_BUS_BUSY;
    }
}

hal_spi_result_t hal_spi_master_get_config(struct hal_spi_context *spi_ctx, struct hal_spi_config *config) {
    return HAL_SPI_ERR_OTHER; // ESP-IDF doesn't provide a way to get configuration
}

hal_spi_result_t hal_spi_master_write(struct hal_spi_context *spi_ctx, const uint8_t *data, size_t len, hal_timeout_ms timeout) {
    if (spi_ctx == NULL || data == NULL || len == 0 || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    BaseType_t mutex_ret_err = xSemaphoreTake(spi_ctx->impl_ctx->mutex, pdMS_TO_TICKS(timeout));
    if (mutex_ret_err == pdTRUE) {
        spi_transaction_t trans = {0};
        trans.length = len * 8; // Length in bits
        trans.tx_buffer = data;
        trans.rx_buffer = NULL;
        
        // Transparent optimization: For sync-and-async mode with DMA support,
        // automatically use DMA for larger transfers, CPU for smaller ones
        spi_device_handle_t handle_to_use = spi_ctx->impl_ctx->device_handle;
        
#if defined(HAL_SPI_ASYNC_DMA_SUPPORT)
        if (spi_ctx->current_mode == HAL_SPI_OP_MODE_SYNC_AND_ASYNC && 
            spi_ctx->impl_ctx->async_device_handle != NULL &&
            len >= 64) { // Use DMA for transfers >= 64 bytes
            handle_to_use = spi_ctx->impl_ctx->async_device_handle;
        }
#endif
        
        hal_spi_result_t spi_result = esp_err_to_spi_hal_err(
            spi_device_transmit(handle_to_use, &trans)
        );
        
        xSemaphoreGive(spi_ctx->impl_ctx->mutex);
        return spi_result;
    } else {
        return HAL_SPI_ERR_BUS_BUSY;
    }
}

hal_spi_result_t hal_spi_master_read(struct hal_spi_context *spi_ctx, uint8_t *data, size_t len, hal_timeout_ms timeout) {
    if (spi_ctx == NULL || data == NULL || len == 0 || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    BaseType_t mutex_ret_err = xSemaphoreTake(spi_ctx->impl_ctx->mutex, pdMS_TO_TICKS(timeout));
    if (mutex_ret_err == pdTRUE) {
        spi_transaction_t trans = {0};
        trans.length = len * 8; // Length in bits
        trans.tx_buffer = NULL;
        trans.rx_buffer = data;
        
        // Transparent optimization: Choose DMA vs CPU based on transfer size
        spi_device_handle_t handle_to_use = spi_ctx->impl_ctx->device_handle;
        
#if defined(HAL_SPI_ASYNC_DMA_SUPPORT)
        if (spi_ctx->current_mode == HAL_SPI_OP_MODE_SYNC_AND_ASYNC && 
            spi_ctx->impl_ctx->async_device_handle != NULL &&
            len >= 64) { // Use DMA for transfers >= 64 bytes
            handle_to_use = spi_ctx->impl_ctx->async_device_handle;
        }
#endif
        
        hal_spi_result_t spi_result = esp_err_to_spi_hal_err(
            spi_device_transmit(handle_to_use, &trans)
        );
        
        xSemaphoreGive(spi_ctx->impl_ctx->mutex);
        return spi_result;
    } else {
        return HAL_SPI_ERR_BUS_BUSY;
    }
}

hal_spi_result_t hal_spi_master_write_read(struct hal_spi_context *spi_ctx, const uint8_t *tx_data, size_t tx_len, uint8_t *rx_data, size_t rx_len, hal_timeout_ms timeout) {
    if (spi_ctx == NULL || spi_ctx->impl_ctx == NULL) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    if ((tx_data == NULL && tx_len > 0) || (rx_data == NULL && rx_len > 0)) {
        return HAL_SPI_ERR_INVALID_ARG;
    }
    
    BaseType_t mutex_ret_err = xSemaphoreTake(spi_ctx->impl_ctx->mutex, pdMS_TO_TICKS(timeout));
    if (mutex_ret_err == pdTRUE) {
        spi_transaction_t trans = {0};
        
        // For simultaneous write/read, lengths must match in ESP-IDF
        size_t transfer_len = (tx_len > rx_len) ? tx_len : rx_len;
        trans.length = transfer_len * 8; // Length in bits
        trans.tx_buffer = tx_data;
        trans.rx_buffer = rx_data;
        
        // Transparent optimization: Choose DMA vs CPU based on transfer size
        spi_device_handle_t handle_to_use = spi_ctx->impl_ctx->device_handle;
        
#if defined(HAL_SPI_ASYNC_DMA_SUPPORT)
        if (spi_ctx->current_mode == HAL_SPI_OP_MODE_SYNC_AND_ASYNC && 
            spi_ctx->impl_ctx->async_device_handle != NULL &&
            transfer_len >= 64) { // Use DMA for transfers >= 64 bytes
            handle_to_use = spi_ctx->impl_ctx->async_device_handle;
        }
#endif
        
        hal_spi_result_t spi_result = esp_err_to_spi_hal_err(
            spi_device_transmit(handle_to_use, &trans)
        );
        
        xSemaphoreGive(spi_ctx->impl_ctx->mutex);
        return spi_result;
    } else {
        return HAL_SPI_ERR_BUS_BUSY;
    }
}