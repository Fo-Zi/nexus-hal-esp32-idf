...
#include "hal_spi_master_async.h"
...
typedef struct {
    ...
    void *callback_ctx;
    hal_spi_result_t *result_ptr;
    struct hal_spi_context *spi_ctx;
} hal_spi_async_transaction_t;

// ESP-IDF transaction completion callback
static void spi_async_transaction_cb(spi_transaction_t *trans) {
    hal_spi_async_transaction_t *async_trans = (hal_spi_async_transaction_t *)trans->user;
    
    if (async_trans && async_trans->callback) {
        hal_spi_result_t result = esp_err_to_spi_hal_err(ESP_OK); // Transaction completed successfully
...
static void hal_async_config_to_esp_config(struct hal_spi_config *config, const struct hal_spi_async_dma_config *async_cfg, spi_device_interface_config_t *esp_config) {
...
static void hal_async_bus_config_to_esp_config(const struct hal_spi_async_dma_config *async_cfg, spi_bus_config_t *esp_bus_config) {
...
hal_spi_result_t hal_spi_enable_async_mode(
    ...
    if (mutex_ret_err == pdTRUE) {
        ...
        if (!spi_ctx->impl_ctx->is_driver_installed) {
            spi_result = HAL_SPI_ERR_INVALID_ARG;
            goto free_mutex_and_ret;
        }
        
        // Update mode to sync-and-async
        spi_ctx->current_mode = HAL_SPI_OP_MODE_SYNC_AND_ASYNC;
...
hal_spi_result_t hal_spi_write_async(
    ...
    if (spi_ctx->current_mode != HAL_SPI_OP_MODE_SYNC_AND_ASYNC || !spi_ctx->async_dma.is_async_initialized) {
        return HAL_SPI_ERR_INVALID_ARG; // Async mode not enabled or not initialized
    }
    ...
    hal_spi_async_transaction_t *async_trans = malloc(sizeof(hal_spi_async_transaction_t));
    if (async_trans == NULL) {
        free(trans);
        return HAL_SPI_ERR_OTHER;
    }
