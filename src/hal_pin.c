...
hal_pin_result_t esp_err_to_hal_err(esp_err_t esp_err){
    switch (esp_err) {
        case ESP_OK:
            return HAL_PIN_OK;
        default:
...
hal_pin_result_t hal_pin_init(hal_pin_context_t * pin_ctxt){
    ...
    if(!isr_service_initialized){
        ...
        hal_pin_result_t result =  esp_err_to_hal_err(gpio_install_isr_service(0));
...
hal_pin_result_t hal_pin_set_config(hal_pin_context_t * pin_ctxt, hal_pin_config_t * config ){
    ...
    hal_pin_result_t result = esp_err_to_hal_err(gpio_config(&esp_pin_config));
    if(result != HAL_PIN_OK){
...
hal_pin_result_t hal_pin_set_state(hal_pin_context_t * pin_ctxt, hal_pin_state_t value){
    hal_pin_result_t result = esp_err_to_hal_err(gpio_set_level(pin_ctxt->pin_num, value));
    if(result != HAL_PIN_OK){
...
hal_pin_result_t hal_pin_set_callback( hal_pin_context_t * pin_ctxt, hal_pin_callback_t callback ){
    hal_pin_result_t result = esp_err_to_hal_err(gpio_isr_handler_add(pin_ctxt->pin_num, callback, NULL));
    if(result != HAL_PIN_OK){
